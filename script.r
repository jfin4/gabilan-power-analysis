...
# log normalize the data, makes means and medians equivalent
...
means_wide_baseline <- means_wide[, (ncol(means_wide) - baseline_years):ncol(means_wide)]
baseline_means <- apply(means_wide_baseline, 1, mean, na.rm = TRUE)
baseline <- mean(baseline_means)
sds_wide <- tapply(data$Result, list(data$StationCode, data$Year), sd)
...
# run model N times on generated data
pvals <- rep(NA, N)
for (i in 1:N) {
    # define interim goal
